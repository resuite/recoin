import { type SourceCell, useObserver } from "retend";
...
export const useIntersectionObserver = (
   element: SourceCell<HTMLElement | null>,
 ...
 * stopping the search if it reaches a parent with the "pull-zone" class.
 * 
 ...
 */
export function getScrollableY(element: HTMLElement,
   ...
   // Continue until we reach the document body or null
   while (current && current !== document.body) {
     if (current === boundary) {
       // Stop searching when we hit a pull-zone element
       return null;
     }
     ...
     if (hasVerticalScroll && current.scrollTop < (current.scrollHeight - current.clientHeight)) {
       return current;
     }
